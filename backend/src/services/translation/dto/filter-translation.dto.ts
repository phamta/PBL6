import { IsOptional, IsString, IsEnum, IsDateString, IsInt, Min, Max } from 'class-validator';
import { ApiPropertyOptional } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import { TranslationStatus } from '@prisma/client';

export class FilterTranslationDto {
  @ApiPropertyOptional({ 
    description: 'Tìm kiếm theo tên người đăng ký hoặc tiêu đề tài liệu', 
    example: 'Nguyễn Văn A' 
  })
  @IsString()
  @IsOptional()
  search?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo trạng thái',
    enum: TranslationStatus 
  })
  @IsEnum(TranslationStatus)
  @IsOptional()
  status?: TranslationStatus;

  @ApiPropertyOptional({ 
    description: 'Lọc theo ngôn ngữ gốc', 
    example: 'Vietnamese' 
  })
  @IsString()
  @IsOptional()
  sourceLanguage?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo ngôn ngữ đích', 
    example: 'English' 
  })
  @IsString()
  @IsOptional()
  targetLanguage?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo loại tài liệu', 
    example: 'Diploma' 
  })
  @IsString()
  @IsOptional()
  documentType?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo mức độ khẩn cấp', 
    enum: ['NORMAL', 'URGENT', 'VERY_URGENT']
  })
  @IsString()
  @IsOptional()
  urgentLevel?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc từ ngày tạo (ISO 8601)', 
    example: '2024-01-01' 
  })
  @IsDateString()
  @IsOptional()
  createdFrom?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc đến ngày tạo (ISO 8601)', 
    example: '2024-12-31' 
  })
  @IsDateString()
  @IsOptional()
  createdTo?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo người tạo', 
    example: 'user-id-123' 
  })
  @IsString()
  @IsOptional()
  createdById?: string;

  @ApiPropertyOptional({ 
    description: 'Lọc theo người duyệt', 
    example: 'user-id-456' 
  })
  @IsString()
  @IsOptional()
  approvedById?: string;

  @ApiPropertyOptional({ 
    description: 'Số trang (bắt đầu từ 1)', 
    example: 1, 
    default: 1 
  })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  @IsOptional()
  page?: number = 1;

  @ApiPropertyOptional({ 
    description: 'Số bản ghi mỗi trang (1-100)', 
    example: 20, 
    default: 20 
  })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  @Max(100)
  @IsOptional()
  limit?: number = 20;

  @ApiPropertyOptional({ 
    description: 'Sắp xếp theo trường (createdAt, updatedAt, urgentLevel)', 
    example: 'createdAt' 
  })
  @IsString()
  @IsOptional()
  sortBy?: string = 'createdAt';

  @ApiPropertyOptional({ 
    description: 'Thứ tự sắp xếp', 
    enum: ['asc', 'desc'],
    default: 'desc' 
  })
  @IsString()
  @IsOptional()
  sortOrder?: 'asc' | 'desc' = 'desc';
}